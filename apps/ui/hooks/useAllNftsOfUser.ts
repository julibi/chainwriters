...
import useFactoryContract from './useMoonpageFactoryContract';
...
const useAllNftsOfUser = () => {
  ...
  const [allProjectAddresses, setAllProjectAddresses] = useState<
    string[] | null
  >(null);
    ...
    if (FactoryContract) {
      try {
        ...
        const projectAddresses =
          multicallResult.results.ALL_DAO_ADDRESSES.callsReturnContext.map(
            (item) => item.returnValues[0].toLowerCase()
          );
  ...
  const fetchFromAlchemyAPI = useCallback(async () => {
    if (
      ...
      const pepoNfts = nfts.ownedNfts.filter((nft) =>
        allProjectAddresses.includes(nft.contract.address.toLowerCase())
      );
