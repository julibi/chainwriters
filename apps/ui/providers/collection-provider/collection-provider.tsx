...
import { BuyArgs, CollectionApi, CollectionProviderProps, MintArgs, StartAuctionsArgs } from './collection-provider.types';
...
export function CollectionProvider({ children }: CollectionProviderProps) {
  ...
  const buy = useCallback(async ({ projectId, initialMintPrice, onSuccess, onError }: BuyArgs) => {
    try {
  ...
  const mint = useCallback(async ({ projectId, amount, price, onSuccess, onError }: MintArgs) => {
      try {
